...
if(isset($_GET["f"]) && isset($_GET["addfolder"])) {
    ...
    if ($addfolder != '' && $addfolder != 'null' && !is_dir($useruploadpath.$parent.$addfolder)) {
...
//                var_dump($dir);
        ...
        for($i=count($files)-1; $i >= 0; $i--):
            ...
            if($file_style == "block") { ?>
                <div class="fileDiv"
                    ...
                    <p class="fileTime"><span class="fileMime">Wymiary: <?=$size[0] ?> x <?=$size[1] ?></span><?php echo $filesizetemp; ?> KB</p>
                </div>
        ...
        if($curr_path != '') {
            file_put_contents('curr_path.txt', $curr_path);
        } else {
            file_put_contents('curr_path.txt', '');
        }
        $folder_info = str_replace('/', ' > ', str_replace(__DIR__.'/uploads/', '', $useruploadpath)).' ';
        if($curr_path != '') {
            $_SESSION['curr_path'] = $curr_path;
        } else {
            $_SESSION['curr_path'] = '';
        }
        echo "
