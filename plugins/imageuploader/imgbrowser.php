...
<div id="deletebar">
    <div id="deletebarDelete" onclick="#" class="editbarDiv"><img src="img/cd-icon-qtrash.png" class="editbarIcon editbarIconLeft"><p class="editbarText"><?php echo $buttons4; ?></p></div>
    <img onclick="hideDeleteBar();" src="img/cd-icon-close-black.png" class="editbarIcon editbarIconRight">
</div>
...
<?php
//var_dump($_POST);
//$value1 = $_POST['size1'] ?? 'value2';
?>
<p class="folderInfo">
    <a href="" class="btnIcon fa fa-home" onclick="reloadImages();"></a>
    <span >></span>
    <span id="folder_info"></span>
    <?php echo $imagebrowser2; ?> <span id="finalcount"></span> <?php echo $imagebrowser3; ?> - <span id="finalsize"></span>
    <?php if($file_style == "block") { ?>
        ...
        <span id="folder_back"></span>
        <span id="folder_add"></span>
        <!--<img title="Post" src="img/cd-icon-done.png" class="headerIcon floatRight" onclick="requestForm({'key1': 'value1'});">
        <img title="Post" src="img/cd-icon-done.png" class="headerIcon floatRight" onclick="requestForm({'key1': 'value2'});">-->
...
<div id="files">
    ...
    if(!isset($_SESSION['curr_path'])){
        $itemnew_name_for_imgfolder = file_get_contents('curr_blogname.txt');
        if ($itemnew_name_for_imgfolder != '' && !is_dir($useruploadpath.$itemnew_name_for_imgfolder)) {
            mkdir($useruploadpath.$itemnew_name_for_imgfolder);
        }
        loadImages($itemnew_name_for_imgfolder);
    } else {
        $current = $_SESSION['curr_path'];
        $current = str_replace('/', '', $current);
        if ($current != '' && !is_dir($useruploadpath.$current)) {
            mkdir($useruploadpath.$current);
        }
        loadImages($current);
    }
...
<div id="uploadImgDiv" class="lightbox popout">
    ...
    <form action="imgupload.php" method="post" enctype="multipart/form-data" id="uploadImgForm" onsubmit="return checkUpload();">
        ...
        <?php
    $dim_arr = ['275x275', '430x310', '960x440'];
    echo '<!-- PAGE ID -->
		<div class="headerBtn">
			<div class="col-lg-2 col-md-2 col-sm-2 text-right">
				<label for="id_page">Wymiary min<span class="required">*</span></label>
			</div>
			<div class="col-lg-4 col-md-4 col-sm-4">
				<select id="dimentions" name="dimentions" onchange="this.value">';
            foreach ($dim_arr as $dim) {
                $select = $dimentions == $dim ? 'selected' : '';
                echo '<option value="'.$dim.'" '.$select.'>'.$dim.'</option>';
            }
        echo '</select>
            </div>
        </div>';
