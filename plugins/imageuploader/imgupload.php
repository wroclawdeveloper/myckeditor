...
list($fname, $ex) = explode('.', $_FILES["upload"]["name"]);
ob_start();
var_dump($fname);
$dane = ob_get_contents();
file_put_contents('debug_data.txt', __FILE__.'~~/'.basename(getcwd()).': '.date('Y-m-d H:i:s').': '.$dane."\n", FILE_APPEND);
ob_end_clean();

$ext = $info['extension'];
//$embed = filter_input(INPUT_POST, 'folder_path', FILTER_SANITIZE_STRING);
$embed = file_get_contents('curr_path.txt');
$target_dir = $useruploadpath.$embed;
...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["upload"]["tmp_name"], $target_file)) {
        ...
        $dim = explode('x', $_POST['dimentions']);
        $thumb = PhpThumbFactory::create($target_file);
        $thumb->adaptiveResizeQuadrant($dim[0], $dim[1]);
        $thumb->save($target_file);
